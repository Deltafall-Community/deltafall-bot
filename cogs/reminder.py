...
class reminder(commands.Cog):
    ...
    async def on_remind_end(self, reminder: Reminder):
        ...
        await bot.get_channel(reminder.channel_id).send(f"{bot.get_user(reminder.author_id).mention} Reminder: `{reminder.message}`", allowed_mentions=allowed_mentions)
    ...
    async def createreminder(self, interaction: discord.Interaction, on: str, message: Optional[str]):
        ...
        reminder = Reminder(interaction.channel.id, interaction.user.id)
        if message: reminder.message = message
        await self.scheduler.add_payload(interaction.guild.id, on, reminder)
        await interaction.followup.send(f"Reminder: `{reminder.message}` going off in <t:{str(int(on.timestamp()))}:R>.")
