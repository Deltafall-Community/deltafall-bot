...
async def get_quote_id(connection, table, id):
    ...
    if quote:
        return Quote(quote[0], quote[1], quote[-1])
...
class randomquote(commands.Cog):
    ...
    id: Optional[int]):
        ...
        if id:
            quote = await get_quote_id(connection, table=interaction.guild.id, id=id)
        else:
            quote = await get_random_quote(connection, table=interaction.guild.id)
    ...
    async def on_message(self, message):
        if (message.content.lower() == "aq" or message.content == "<@949479338275913799>") and message.reference:
            messager = await message.channel.fetch_message(message.reference.message_id)
            if not await self.bot.setting_manager.get_user_setting(messager.author, ("quote", "allow")):
                return
            ...
            for role in message.author.roles:
                if role.id == 1330210181187113051:
                    return await message.reply("erm you mighht been banned fom quoting idk", allowed_mentions=discord.AllowedMentions.none())
            if messager.content == "" and not messager.attachments:
                return await message.reply("its just an empty text you idiot", allowed_mentions=discord.AllowedMentions.none())
            content = messager.content
            display_content = content
            if len(messager.content) > 500:
                display_content = f'{messager.content[:500]}...'
