...
def parse_from_soundcloud_api(track: Track):
    ...
    if metadata.created_on: metadata.created_on = datetime.fromisoformat(metadata.created_on)
...
def parse_from_ytdlp_dict(ytdlp_dict: dict, add_stream_url: bool = True):
    ...
    if metadata.created_on: metadata.created_on = datetime.fromtimestamp(metadata.created_on)
    ...
    if not metadata.url: metadata.url=ytdlp_dict.get("url")
    elif add_stream_url: metadata.stream_url=ytdlp_dict.get("url")
...
def get_metadata(search: str) -> List[Metadata]:
    ...
    if urlparse(search).netloc != '': url = search
    else: raw_songs = [ydl.extract_info(f"ytsearch1:{search}", download=False).get("entries")[0]]
    
    extractor=""
    if url:
        extractors = yt_dlp.extractor.gen_extractors()
        for e in extractors:
            if e.suitable(url) and e.IE_NAME != 'generic': extractor = (e.IE_NAME+" ")[:e.IE_NAME.find(":")]
    ...
    match extractor:
        ...
        case "youtube" | "bandcamp":
            if not raw_songs:
                ...
                if entries: raw_songs += entries
                else: raw_songs += [yt_dlp_dict]
        # special case
        case "soundcloud":
            soundcloud_music = soundcloud_api.resolve(url)
            music_type = type(soundcloud_music)
            if music_type is Playlist:
                for track in soundcloud_music.tracks: songs.append(parse_from_soundcloud_api(track))
            elif music_type is Track:
                songs.append(parse_from_soundcloud_api(soundcloud_music))

    for song in raw_songs: songs.append(parse_from_ytdlp_dict(song, not extractor in ("bandcamp",)))
    return songs
